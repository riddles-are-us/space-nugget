...
async function main() {
  ...
  try {
    ...
    let nonce_bid = await player_bid.getNonce();
    await player_bid.runCommand(commandSpec.BID_NUGGET.id, nonce_bid, [
      BigInt("1"),
      10n,
    ]);
  } catch (e) {
  ...
  console.log("Start run LIST_NUGGET ...");
  let nonce_bid = await player_bid.getNonce();
  await player_bid.runCommand(commandSpec.LIST_NUGGET.id, nonce_bid, [0n, 15n]);
